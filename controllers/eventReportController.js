...
export const getRevenueReport = asyncHandler(async (req, res) => {
  ...
  if (group_by === "day") {
    ...
    throw new ApiError("Invalid group_by parameter", 400)
  }
...
/**
 * @desc    Get event type performance report
 * @route   GET /api/events/reports/event-type-performance
 * @access  Private
 */
export const getEventTypePerformance = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(
    new ApiResponse(
      ...
      "Event type performance report generated successfully",
    ),
  )
})
 ...
 * @route   GET /api/events/reports/venue-utilization
 * @access  Private
 */
export const getVenueUtilization = asyncHandler(async (req, res) => {
  ...
  const totalUtilizationRate = totalAvailableHours > 0 ? ((totalBookedHours / totalAvailableHours) * 100).toFixed(2) : 0
 ...
 * @route   GET /api/events/reports/service-popularity
 * @access  Private
 */
...
/**
 * @desc    Get customer satisfaction report
 * @route   GET /api/events/reports/customer-satisfaction
 * @access  Private
 */
export const getCustomerSatisfaction = asyncHandler(async (req, res) => {
  ...
  const formattedCategoryRatings = {
    venue: categoryRatings.venue.count > 0 ? (categoryRatings.venue.total / categoryRatings.venue.count).toFixed(2) : 0,
    staff: categoryRatings.staff.count > 0 ? (categoryRatings.staff.total / categoryRatings.staff.count).toFixed(2) : 0,
    food: categoryRatings.food.count > 0 ? (categoryRatings.food.total / categoryRatings.food.count).toFixed(2) : 0,
    value: categoryRatings.value.count > 0 ? (categoryRatings.value.total / categoryRatings.value.count).toFixed(2) : 0,
  }
